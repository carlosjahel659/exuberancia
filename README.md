...
La barra de regreso es fija solo dentro de la categoría abierta. La navegación móvil cerrada no deja enlaces invisibles en el recorrido del teclado. Los controles tienen áreas táctiles de al menos 44 px. Las animaciones respetan `prefers-reduced-motion`.
...
Verificación local del 7 de septiembre de 2026: compilación correcta, 19 pruebas unitarias y 10 pruebas de navegador aprobadas. Chrome se verificó a 320, 375, 390, 768, 1024, 1440 y 1920 px, sin desbordamiento horizontal. Se comprobaron consola, variantes, seis promociones, teclado, áreas táctiles, enlaces, HTML sin JavaScript, metadatos y recursos bajo `/exuberancia/`. Axe no detectó infracciones WCAG A/AA en portada, desayunos, bebidas y barbacoa. Se inspeccionaron capturas de escritorio y móvil; no equivale a probar Safari ni dispositivos físicos.

La auditoría npm con acceso a la red reportó cero vulnerabilidades después de actualizar Vite. El CSS compilado pasó de 50,343 a 30,543 bytes, y JavaScript de 249,716 a 224,477 bytes, comparados con los archivos compilados disponibles antes del rediseño. Estos tamaños no son una medición de tiempo de carga en red móvil. `src/data/menu.js`, las imágenes y el PDF no tienen cambios.
