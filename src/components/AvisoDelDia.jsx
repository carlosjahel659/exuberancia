 ...
 */
export default function AvisoDelDia({ ahora }) {
  const aviso = avisoDelDia(ahora)

  return (
    <div className="relative overflow-hidden rounded-2xl border border-turquesa/25 bg-turquesa/[0.06] p-5 sm:p-6">
      ...
      >
        <p className="flex shrink-0 items-center gap-2.5 text-turquesa">
          {RELOJ}
          <span className="font-alt text-xl uppercase leading-none tracking-[0.1em] sm:text-2xl">
            {aviso.saludo}
            <span className="ml-2 text-crema/55">{aviso.reloj}</span>
          </span>
        </p>

        <p className="text-sm leading-relaxed text-crema/85 sm:text-[15px]">{aviso.texto}</p>
      </div>
